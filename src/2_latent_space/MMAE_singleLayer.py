...
EMBEDDING_SPACE = 'MMAE_embedding_CAT' # 'MMAE_embedding_3' 
CATEGORIC = 1
...
# narrative
#df_narrative= pd.read_csv('/exports/reum/tdmaarseveen/RA_Clustering/new_data/5_clustering/df_narrative_RA.csv', sep=',')
#l_narrative = [col for col in df_narrative.columns if col not in ['pseudoId']] 
#df_narrative = df_narrative[l_narrative]

if CATEGORIC == 1:
    df_categoric = pd.read_csv('/exports/reum/tdmaarseveen/RA_Clustering/new_data/5_clustering/df_categoric_wAge.csv', sep=',') # _wAge
    Z = df_categoric[df_categoric.columns[1:]].copy()
    
    # Set network parameters
    input_shapes = {'Numeric': Z.shape[1:]} # [df_categoric.shape[1:], (df_numeric.shape[1:])]

    data = [np.array(Z.values)] 
    ...
    loss = ['bernoulli_divergence'] 
    
    
elif CATEGORIC == 0:
    df_numeric = pd.read_csv('/exports/reum/tdmaarseveen/RA_Clustering/new_data/5_clustering/df_lab_scaled_demographics.csv', sep=',')[['Leuko', 'MCH', 'Hb', 'Ht', 'MCHC',  'MCV', 'Trom', 'BSE',  'Age']]
    Z = df_numeric.copy()
    
    # Set network parameters
    input_shapes = {'Numeric': Z.shape[1:]}
...
history = autoencoder.fit(data, epochs=100, batch_size=256, validation_split=0.1) # validation_data=validation_data # 1000 or 5000
...
pyplot.legend()
pyplot.savefig('/exports/reum/tdmaarseveen/RA_Clustering/figures/3_clustering/MMAE_convergence_model_CATEGORICAL.png', dpi=100)
...
# Writing to file
#np.savetxt('/exports/reum/tdmaarseveen/RA_Clustering/figures/3_clustering/MMAE_embedding_CAT.txt', z_filtered) 
...
visualize_umap_bokeh_na(X_embedded, df_imp, list(df_imp.columns), l_binary=l_cat, patient_id='patnr', cluster_id='MMAE_clusters', title='MMAE_UMAP',  path='/exports/reum/tdmaarseveen/RA_Clustering/TSNE/MMAE_CAT_ONLY.html')
